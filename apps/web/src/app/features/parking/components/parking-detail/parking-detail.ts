...
export class ParkingDetail {
  ...
  protected readonly occupiedSlots = computed(() =>
    Math.round(
      (this.totalSlots() * (this.parking()?.occuppationRate || 0)) / 100
    )
  );
  ...
  protected static slotLabel(slot: SlotDistribution): string {
    return [slot.zone, slot.type].filter(Boolean).join(" · ").toUpperCase();
  }

  protected readonly slotLabel = ParkingDetail.slotLabel;

  protected static formattedDate(dateStr: string): string {
  ...
  protected readonly formattedDate = ParkingDetail.formattedDate;
