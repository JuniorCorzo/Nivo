...
interface ComboboxAccessor {
  combobox?: () => {
    _deferredContentAware?: {
      contentVisible: { set: (v: boolean) => void };
    };
    open: () => void;
  } | null;
}
...
describe("ComboboxComponent", () => {
  ...
  const openDropdown = () => {
    ...
    const target: unknown = fixture.componentInstance;
    /* SAFETY: Accessing internal combobox signal on component in test */
    const instance = target as ComboboxAccessor;
    const combobox = instance.combobox?.();
    if (combobox) {
  ...
  describe("Filtering", () => {
    ...
    it("should show no results when search matches nothing", () => {
      ...
      expect(getPopupText()).toContain("Sin resultados");
    });
  });
  ...
  describe("Keyboard navigation", () => {
    ...
    it("should highlight previous item on ArrowUp", () => {
      ...
      const options = getOptions();
      expect(options[0].dataset["active"]).toBe("true");
      expect(options[1].dataset["active"]).toBe("false");
    });
  ...
  describe("Selection via mouse", () => {
    ...
    it("should select an item on mousedown and emit selectionChange", () => {
      ...
      const options = getOptions();
      /* SAFETY: Option element in list is an HTMLElement */
      const optionEl = options[1] as HTMLElement;
  ...
  describe("States", () => {
    ...
    it("should show error message inside dropdown when error is set and dropdown is open", () => {
      ...
      expect(getPopupText()).toContain("Error al cargar departamentos");
    });
    ...
    it("should show error message below input when dropdown is closed", () => {
      ...
      expect(alert.textContent).toContain("Error al cargar departamentos");
    });
